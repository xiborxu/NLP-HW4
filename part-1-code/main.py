...
def create_augmented_dataloader(args, dataset):
    ...
    augmented_dataset = train_dataset.select(augmented_indices)
    transformed_dataset = augmented_dataset.map(custom_transform, load_from_cache_file=False)
    combined_dataset = datasets.concatenate_datasets([train_dataset, transformed_dataset])
